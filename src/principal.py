...
def main(uf: str) -> int:
    ...
    area_inundada  = dados["_metadata"].get("area_inundada")
    ...
    if estado["state"] < ETAPA_TERRENO:
        ...
        # Apenas guarda na memória o que deve ser salvo depois
    ...
    if not simulacao_atualizada:
        ...
        # Apenas guarda na memória (sobrescreve a máscara com a versão expandida)
        arquivos_para_salvar["mascara_agua"] = mascara_agua
    ...
    # ------------------------------------------------------------------
    # 6. PERSISTÊNCIA — ÚNICO PONTO DE GRAVAÇÃO NO FINAL DO PROGRAMA
    # ------------------------------------------------------------------
    if arquivos_para_salvar or metadados_para_salvar:
        print("\n[main] Gravando dados encapsulados no arquivo ao final da execução...")
        estado["uf"]       = uf
        estado["elevacao"] = elevacao
        
        # Chama a persistência uma única vez, enviando todo o "pacotão" de modificações
